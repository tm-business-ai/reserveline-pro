...
- 直近予約の確認
- 直近予約のキャンセル
- 電話・店頭予約の登録・変更（電話番号、予約経路、空き時間候補からの登録）
- 既存予約の変更（日時、メニュー、予約状態、備考）
- 本日の予約・今週の予約表示
- キャンセルされた予約の表示
- 予約データをダウンロード
- 予約メニューの設定
...
予約経路は、LINE、電話、店頭、管理画面、その他を区別して管理できます。電話・店頭予約では電話番号を任意で登録できます。
電話・店頭予約は、予約ルールと既存予約を反映した空き時間から選んで登録できます。既存予約の内容や予約状態も管理画面から変更できます。
...
管理画面では、予約ルールの設定、営業日・営業時間・予約間隔・臨時休業日の管理、日付とメニューを指定した空き時間の確認ができます。臨時休業日は一覧から有効／無効の変更と複数削除ができます。

LINEから予約が入った時に管理者のLINEまたは店舗用LINEグループへ通知する機能は、今後の拡張予定です。現時点では、予約は管理画面で確認する設計です。本番運用では、管理者通知、エラーログ、通知失敗時の再送管理が必要です。
